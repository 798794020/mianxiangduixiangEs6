...
class Tab{
	constructor(str){
		...
		this.oBtn=this.parent.querySelectorAll('input');
		this.oLi=this.parent.querySelectorAll('li');
		this.show();
	}
	show(){
		let _this=this;
...
class AutoPlay extends Tab{
	...
	autoFn(){
		...
		_this.timer=setInterval(function(){
			_this.index++;
			if(_this.index==_this.oLi.length){
				_this.index=0;
			}
...
class BtnBox extends AutoPlay{
	constructor(str){
		...
		clearInterval(_this.timer)
		//console.log(_this.oLi.length)
		if(_this.index==_this.oLi.length-1){
			_this.index=-1;
		}
		_this.index++
		_this.autoFn()
	...
	autoLeft(){
		if(this.index==0){
			...
			_this.index--;
			if(_this.index<0){
				_this.index=_this.oLi.length-1;
			}
			_this.fn(_this.index)
		},2000)
	}
